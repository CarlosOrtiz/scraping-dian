...
@Processor('dian')
export class TaxReturnProcessor {
  private readonly logger = new Logger(TaxReturnProcessor.name);
  constructor(
    @InjectRepository(Audit, 'security') private readonly auditRepository: Repository<Audit>,
  ) { }

  @Process({ name: 'taxReturn' })
  async taxReturn(job: Job<any>) {
