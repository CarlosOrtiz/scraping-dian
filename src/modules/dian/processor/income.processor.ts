...
export class IncomeProcessor {
  ...
  async rentalDeclaration(job: Job<any>, callback: DoneCallback) {
    ...
    try {
        ...
        if (dashboardForm[0]) {
          ...
          if (body.year_Rental_Declaration === 2019) {
            ...
            if (buttonAll) {
              ...
              const tax_resident_1 = await body.tax_resident_1 === true ? await popupQuestionFromItem2[0] : await popupQuestionFromItem2[1]
              await browser.pause(3000);
              const severance_pay_2016_2 = await body.severance_pay_2016_2 === true ? await popupQuestionFromItem2[2] : await popupQuestionFromItem2[3]
              await browser.pause(3000);
              const public_server_3 = await body.public_server_3 === true ? await popupQuestionFromItem2[4] : await popupQuestionFromItem2[5]
              await browser.pause(3000);
              const income_country_4 = await body.income_country_4 === true ? await popupQuestionFromItem2[6] : await popupQuestionFromItem2[7]
              await browser.pause(3000);
              ...
              if (inputText[0]) {
                const panelBUttonSave = await browser.$$('div[class="fixed-action-btn click-to-toggle"] a')
                if (panelBUttonSave[0]) {

                  await panelBUttonSave[0].doubleClick();
                  await browser.pause(3000);
      ...
      uploadFileIncome = await this.UploadFileGDrive(dirIncome, ('RENTA-' + job.id));
      console.log(`FILE UPLOADED WITH NAME: RENTA-${job.id}`)
